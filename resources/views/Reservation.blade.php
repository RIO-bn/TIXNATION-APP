...
<nav class="Nav" >
    ...
    @if($foods->isEmpty())
        ...
        <input type="hidden" name="foods" value="[]"> <!-- Always send empty array -->
    @else
    <h3 class="Choose">Choose Foods</h3>
    <div class="food-container">
           
        <div class="food-column">
            @foreach ($foods->take(ceil($foods->count() / 2)) as $index => $food)
                <div class="food-item" data-price="{{ $food->price }}">
                    <label>{{ $food->name }} - Rp {{ number_format($food->price, 0, ',', '.') }}</label>
                    <input type="number" name="foods[{{ $index }}][quantity]" min="0" value="0" class="form-control quantity-input">
                    <input type="hidden" name="foods[{{ $index }}][id]" value="{{ $food->id }}">
                </div>
            @endforeach
        </div>

        <div class="food-column">
            @foreach ($foods->skip(ceil($foods->count() / 2)) as $index => $food)
                <div class="food-item" data-price="{{ $food->price }}">
                    <label>{{ $food->name }} - Rp {{ number_format($food->price, 0, ',', '.') }}</label>
                    <input type="number" name="foods[{{ $loop->index + ceil($foods->count() / 2) }}][quantity]" min="0" value="0" class="form-control quantity-input">
                    <input type="hidden" name="foods[{{ $loop->index + ceil($foods->count() / 2) }}][id]" value="{{ $food->id }}">
                </div>
            @endforeach
        </div>
    </div>
@endif
        <!-- Total Price Display -->
         <p>please select NULL if you dont want to order any foods</p>
    ...
    <a href="{{ route('index') }}"
   class="inline-block bg-orange-600 text-white font-semibold px-6 py-3 rounded-xl shadow-lg hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-400 transition duration-300 no-underline">
